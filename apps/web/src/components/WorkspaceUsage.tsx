...
export function WorkspaceUsage({ initialSnapshot, workspaceId }: Props) {
  ...
  return <div className="grid gap-6">
    ...
    <div className="grid gap-6 xl:grid-cols-2"><Card><CardHeader><CardTitle className="flex items-center gap-2"><Activity className="size-5 text-brand" />Workspace activity</CardTitle><CardDescription>{formatDateTime(snapshot.range.start)} to {formatDateTime(snapshot.range.end)} · {snapshot.range.semantics}</CardDescription></CardHeader><CardContent className="grid gap-3"><div className="flex items-center justify-between rounded-lg border border-border p-4"><span>Ingestion failures</span><StatusBadge status={snapshot.historical.ingestion_failures > 0 ? "warning" : "success"}>{integer.format(snapshot.historical.ingestion_failures)}</StatusBadge></div>{snapshot.historical.activity.length === 0 ? <EmptyState description="No activity observations are available for this interval." icon={Activity} title="No recorded activity" /> : snapshot.historical.activity.map((item) => <div className="flex items-center justify-between gap-4 rounded-lg border border-border p-4" key={`${item.event_kind}-${item.outcome ?? "none"}`}><span className="text-sm">{item.event_kind.replaceAll("_", " ")}{item.outcome ? ` · ${item.outcome.replaceAll("_", " ")}` : ""}</span><strong>{integer.format(Number(item.aggregate_count))}</strong></div>)}</CardContent></Card>

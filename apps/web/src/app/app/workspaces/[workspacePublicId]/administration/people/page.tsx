...
export default async function PeoplePage({ params }: Readonly<{ params: Promise<{ workspacePublicId: string }> }>) {
  ...
  return <div className="grid gap-6"><header><p className="text-sm font-bold uppercase tracking-[0.14em] text-brand">Administration</p><h1 className="mt-2 text-3xl font-semibold">People &amp; roles</h1><p className="mt-2 text-foreground-muted">See who can access this workspace and make authorised role changes.</p></header><WorkspaceAdministration actorRole={workspace.role} initialSnapshot={administration} view="people" workspaceId={workspacePublicId} /></div>;
}

...
final class GetWorkspaceUsage
{
    ...
    public function handle(Workspace $workspace, string $range): array
    {
        ...
        return [
            ...
            'gauges' => $gauges,
            'historical' => [
                'ingestion_failures' => DB::table('ingestion_event_claims')->where('workspace_id', $workspace->id)->where('status', 'failed')->where('failed_at', '>=', $start)->where('failed_at', '<', $end)->count(),
                'activity' => $activities,
                'usage' => $usage,
            ],
            'labels' => [

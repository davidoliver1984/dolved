...
final class V3EngineeringExecutionHarnessTest extends TestCase
{
    ...
    {
        $source = app(Exp0008EngineeringBenchmarkSource::class);
        $document = $source->load()['provisioning']['documents'][0];
        $contents = file_get_contents(Exp0008EngineeringBenchmark::root().'/source/'.$document['source_path']);
    ...
    public function test_v2_and_v3_identities_and_state_paths_are_separate(): void
    {
        ...
        $this->assertSame('3', Exp0008EngineeringBenchmark::VERSION);
        $this->assertNotSame(EngineeringBenchmark::WORKSPACE_SLUG, Exp0008EngineeringBenchmark::WORKSPACE_SLUG);
    ...
    public function test_incomplete_v3_state_cannot_claim_materialisation(): void
    {
        ...
        $state = $states->write([
            'schema_version' => 'v1',
            'status' => 'PROVISIONING',
            'benchmark' => ['population_digest' => Exp0008EngineeringBenchmark::POPULATION_DIGEST],
            'provisioning_definition_digest' => Exp0008EngineeringBenchmark::PROVISIONING_DEFINITION_DIGEST,
        ]);
    ...
    {
        $locations = collect(app(Exp0008EngineeringBenchmarkSource::class)->load()['provisioning']['locations'])->keyBy('location_id');

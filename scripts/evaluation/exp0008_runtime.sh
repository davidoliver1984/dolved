...
readonly run_id='EXP-0008-v3-final-engineering-confirmation'
readonly project_name='rag-platform'
readonly repository_root="$(git rev-parse --show-toplevel)"
readonly compose_override="${repository_root}/compose.exp0008.yaml"
readonly definition="${repository_root}/tests/evaluation/experiment-definitions/${run_id}/runtime-lineage.json"
readonly expected_record_sha="$(jq -er '.materialisation_reuse.record_sha256' "${definition}")"
readonly environment_file="${EXP0008_ENV_FILE:-/Users/davidoliver/Desktop/RAG/.env}"
readonly provisioning_record="${EXP0008_PROVISIONING_RECORD:-/private/tmp/rag-platform-exp0008/provisioning.json}"
readonly runs_root="${EXP0008_RUNS_ROOT:-/private/tmp/rag-platform-exp0008/runs}"
readonly index_file="${EXP0008_INDEX_FILE:-/private/tmp/rag-platform-exp0008/index/EXPERIMENTS.md}"
...
assert_inputs() {
    [[ -f "${environment_file}" ]] || { printf 'Missing EXP-0008 environment file.\n' >&2; exit 1; }
    [[ -f "${provisioning_record}" ]] || { printf 'Missing EXP-0008 provisioning record.\n' >&2; exit 1; }
    [[ "$(sha256 "${provisioning_record}")" == "${expected_record_sha}" ]] || {
        printf 'The EXP-0008 provisioning record failed closed on SHA-256.\n' >&2
        exit 1
    }
}

compose() {
    EXP0008_PROVISIONING_RECORD="${provisioning_record}" \
    EXP0008_RUNS_ROOT="${runs_root}" \
    EXP0008_INDEX_FILE="${index_file}" docker compose \
...
prepare() {
    ...
    [[ "$(sha256 "${source}")" == "${expected_record_sha}" ]] || {
        printf 'The materialisation source does not match EXP-0008.\n' >&2
        exit 1
    }
    mkdir -p "$(dirname "${provisioning_record}")"
    install -m 0444 "${source}" "${provisioning_record}"
    printf 'Prepared verified EXP-0008 materialisation record at %s\n' "${provisioning_record}"
}
...
verify() {
    ...
    compose exec -T api php artisan evaluation:benchmark:preflight-exp-0008
    ...
    verify_queues
    printf 'Verified provider-free EXP-0008 preflight at %s.\n' "$(git -C "${repository_root}" rev-parse HEAD)"
}
...
close() {
    ...
    [[ -f "${run_directory}/application-observations.json" ]] || {
        printf 'Missing preserved EXP-0008 observations under %s\n' "${runs_root}" >&2
        exit 1
    }
    [[ "$(jq -er '.observations | length' "${run_directory}/application-observations.json")" == '31' ]] || {
        printf 'EXP-0008 cannot close without exactly 31 observations.\n' >&2
        exit 1
    }
    compose run --rm --no-deps -T --env PYTHONPATH=/app ai \
        ...
        --planner-prompt-version adr-0022-v5 \
    ...
    compose run --rm --no-deps -T --env PYTHONPATH=/app ai \
        ...
        --planner-expectations /evaluation/engineering/expectations.json
    [[ "${first}" == "$(openssl dgst -sha256 "${run_directory}/result.json" "${run_directory}/config.json" "${run_directory}/comparison.json")" ]] || {
        printf 'EXP-0008 deterministic compilation changed between passes.\n' >&2
        exit 1
    }
    compose run --rm --no-deps -T --env PYTHONPATH=/app ai \
...
run() {
    ...
    local head
    head="$(git -C "${repository_root}" rev-parse HEAD)"
    compose exec -T api php artisan evaluation:benchmark:run-exp-0008 --repository-commit="${head}"
    close
}

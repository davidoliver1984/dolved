...
readonly run_id='EXP-0006-adr0022-v4-consolidated-engineering-confirmation'
readonly project_name='rag-platform'
readonly repository_root="$(git rev-parse --show-toplevel)"
readonly compose_override="${repository_root}/compose.exp0006.yaml"
...
readonly environment_file="${EXP0006_ENV_FILE:-${repository_root}/.env}"
readonly provisioning_record="${EXP0006_PROVISIONING_RECORD:-/private/tmp/rag-platform-exp0006/provisioning.json}"
readonly runs_root="${EXP0006_RUNS_ROOT:-${repository_root}/docs/evaluation/runs}"
readonly index_file="${EXP0006_INDEX_FILE:-${repository_root}/docs/evaluation/EXPERIMENTS.md}"
...
assert_runtime_inputs() {
    [[ -f "${environment_file}" ]] || { printf 'Missing EXP-0006 environment file: %s\n' "${environment_file}" >&2; exit 1; }
    [[ -f "${provisioning_record}" ]] || { printf 'Missing EXP-0006 provisioning record: %s\n' "${provisioning_record}" >&2; exit 1; }
    [[ "$(sha256 "${provisioning_record}")" == "${expected_record_sha}" ]] || {
        printf 'The EXP-0006 provisioning record failed closed on its SHA-256 digest.\n' >&2
        exit 1
    }
}

compose() {
    EXP0006_PROVISIONING_RECORD="${provisioning_record}" \
    EXP0006_RUNS_ROOT="${runs_root}" \
    EXP0006_INDEX_FILE="${index_file}" \
    docker compose \
...
prepare() {
    ...
    [[ "$(sha256 "${source}")" == "${expected_record_sha}" ]] || {
        printf 'The provisioning source does not match the immutable EXP-0006 definition.\n' >&2
        exit 1
    }
    mkdir -p "$(dirname "${provisioning_record}")"
    install -m 0444 "${source}" "${provisioning_record}"
    printf 'Prepared verified EXP-0006 provisioning record at %s\n' "${provisioning_record}"
}
...
verify() {
    ...
    compose exec -T api php artisan evaluation:benchmark:preflight-exp-0006
    ...
    verify_queues
    printf 'Verified provider-free EXP-0006 preflight at %s.\n' "$(git -C "${repository_root}" rev-parse HEAD)"
}
...
run() {
    ...
    local head
    head="$(git -C "${repository_root}" rev-parse HEAD)"
    compose exec -T api php artisan evaluation:benchmark:run-exp-0006 --repository-commit="${head}"
    close
}

close() {
    [[ -f "${runs_root}/${run_id}/application-observations.json" ]] || {
        printf 'Missing preserved EXP-0006 observations under %s\n' "${runs_root}" >&2
        exit 1
    }
    [[ -f "${runs_root}/EXP-0005-adr0022-v2-consolidated-engineering-baseline/result.json" ]] || {
        printf 'Missing immutable EXP-0005 comparison result under %s\n' "${runs_root}" >&2
        exit 1
    }
    ...
    compose run --rm --no-deps -T --env PYTHONPATH=/app ai \
        ...
        --baseline-result /evaluation-runs/EXP-0005-adr0022-v2-consolidated-engineering-baseline/result.json
    compose run --rm --no-deps -T --env PYTHONPATH=/app ai \
